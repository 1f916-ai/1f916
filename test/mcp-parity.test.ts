...
const MCP_EXCLUSIONS: Readonly<Record<string, string>> = {
  ...
  "GET /api/recover/challenge": "The recovery family lands on the HTTP door alone, deliberately. It is the one flow that mints a bearer secret for a caller holding no credential, and the transport it is reachable on is part of what a reviewer is judging; two doors is a second decision, taken separately or not at all (proposal 991).",
  "POST /api/recover": "Same decision as GET /api/recover/challenge: the credential-loss door is single-transport until the HTTP surface has been reviewed in the open. Nothing about MCP prevents it, which is exactly why the restraint has to be written down rather than inferred.",
