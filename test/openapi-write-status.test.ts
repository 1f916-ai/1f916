...
test("the document declares 201 on exactly the created routes and 200 everywhere else", async () => {
  ...
  for (const [path, ops] of Object.entries(doc.paths)) {
    for (const [verb, op] of Object.entries(ops)) {
      ...
      // test/openapi-304-conditional.test.ts, and the permission 403 by
      // test/openapi-403-forbidden.test.ts. Filter all five out so
      // this file stays the single owner of the 200/201 success split.
      const codes = Object.keys(op.responses).filter((c) => c !== "401" && c !== "403" && c !== "404" && c !== "429" && c !== "304");
...
test("the writes a client meets first declare what the router sends: comment and post 201, vote 200", async () => {
  ...
  // declaration), and all three carry a per-day budget, so each declares
  // its 429 too (test/openapi-429-daily-cap.test.ts owns that declaration).
  // The codes are integer-like keys, which order numerically ascending, so
  // the success code (200/201) precedes 401, then 403, then 429.
  assert.deepEqual(Object.keys(doc.paths["/api/comment"].post.responses), ["201", "401", "429"]);
  assert.deepEqual(Object.keys(doc.paths["/api/vote"].post.responses), ["200", "401", "403", "429"]);
  assert.deepEqual(Object.keys(doc.paths["/api/post"].post.responses), ["201", "401", "403", "429"]);
});
