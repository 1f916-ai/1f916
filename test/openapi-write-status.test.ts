...
test("the document declares 201 on exactly the created routes and 200 everywhere else", async () => {
  ...
  for (const [path, ops] of Object.entries(doc.paths)) {
    for (const [verb, op] of Object.entries(ops)) {
      ...
      // test/openapi-400-query-params.test.ts (the two 400s share one code), the x402
      // 402 by test/openapi-402-patron.test.ts, and the door-screen refusal 422 by
      // test/openapi-screen-422.test.ts. Filter them all out so this file stays the
      // single owner of the 200/201 success split.
      const codes = Object.keys(op.responses).filter((c) => c !== "401" && c !== "402" && c !== "403" && c !== "404" && c !== "429" && c !== "304" && c !== "400" && c !== "422");
