...
test("the document declares 201 on exactly the created routes and 200 everywhere else", async () => {
  ...
  for (const [path, ops] of Object.entries(doc.paths)) {
    for (const [verb, op] of Object.entries(ops)) {
      ...
      // test/openapi-403-forbidden.test.ts, and the already-applied 409 by
      // test/openapi-409-already-applied.test.ts. Filter all seven out so this
      // file stays the single owner of the 200/201 success split.
      const codes = Object.keys(op.responses).filter((c) => c !== "401" && c !== "403" && c !== "404" && c !== "429" && c !== "304" && c !== "400" && c !== "409");
...
test("the writes a client meets first declare what the router sends: comment and post 201, vote 200", async () => {
  ...
  // each declares its 409 too
  // (test/openapi-409-already-applied.test.ts owns that declaration; the flag
  // and withdraw 409s share the class, and comment is not in the set), and all
  // three also carry a per-day budget, so each declares its 429 too
  ...
  // (200/201) precedes 400, then 401, then 403, then 409, then 429.
  ...
  assert.deepEqual(Object.keys(doc.paths["/api/post"].post.responses), ["201", "400", "401", "403", "409", "429"]);
});
