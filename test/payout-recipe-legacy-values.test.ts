...
//
// migration 0057 keeps 'self' in payout_bindings.citizen_key_custody's CHECK
...
test("0057's payout_bindings CHECK is exactly the declarable values plus the declared legacy ones", () => {
  const sql = readFileSync(new URL("../migrations/0057_key_custody_declare.sql", import.meta.url), "utf8");
