...
import { CHECKPOINT_PROOF_404_ROUTES, PLAIN_404_ROUTES } from "../src/connect.ts";
...
test("the two proof reads declare the plain 404, and the set adds exactly two to the fourteen declared", async () => {
  ...
  for (const [path, ops] of Object.entries(doc)) {
    for (const [verb, op] of Object.entries(ops)) {
      ...
      const expected404 = isProof || isPlainLookup || isProse404 || ((path === "/api/post/{id}" || path === "/api/comment/{id}") && isTyped);
      assert.equal(
        has404,
        expected404,
  ...
  assert.equal(declared404, 16, `expected sixteen declared 404s, got ${declared404}`);
});
