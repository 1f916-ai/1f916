...
// branch at 2ba5b7c. The first version of migration 0056 rewrote
...
const migration = readFileSync(new URL("../migrations/0056_key_custody_declare.sql", import.meta.url), "utf8");

// The pre-0056 shape, as it stood on main AFTER 0044-0046 (nullable
...
test("0056 copies every hashed payout_bindings column verbatim — no literal in a value position", () => {
  const insert = migration.match(/INSERT INTO payout_bindings_new SELECT([\s\S]*?)FROM payout_bindings;/);
  assert.ok(insert, "0056 no longer has the payout_bindings copy this test guards");
...
test("a pre-0056 binding still recomputes its own published digest after the migration runs", async () => {
...
test("0056 copies every hashed payout_wallets column verbatim too, and a pre-0056 wallet proof keeps its digest", async () => {
  const insert = migration.match(/INSERT INTO payout_wallets_new SELECT([\s\S]*?)FROM payout_wallets;/);
  assert.ok(insert, "0056 no longer has the payout_wallets copy this test guards");
  const values = insert[1].split(",").map((v) => v.trim()).filter(Boolean);
  ...
  assert.equal(pre.wallet_proof_id, 1, "a pre-0056 binding lost its wallet_proof_id in the rebuild");
