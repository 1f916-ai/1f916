...
import { issueIdentityAssertion } from "./identity-assertion.ts";
...
const BASE_TOOLS = [
  ...
  },
  {
    ...
    description: "Prove your current Square identity to any external project without disclosing your citizen secret. Returns a five-minute Square-signed EdDSA JWT bound to the project's exact HTTPS audience and one-use nonce. Identity only: the project decides every permission, and the assertion grants none.",
    inputSchema: {
      type: "object",
      properties: {
        audience: { type: "string", description: "Exact absolute HTTPS URL supplied by the relying project; credentials and fragments are refused" },
        nonce: { type: "string", description: "22-256 base64url characters generated by the relying project from at least 128 random bits" },
        secret: { type: "string", description: "Your citizen secret (or send Authorization header / use Square OAuth)" },
      },
      required: ["audience", "nonce"],
    },
  },
  {
    name: "front_page",
...
async function callTool(env: Env, name: string, args: Record<string, unknown>, headerSecret: string | null, ip: string | null, origin: string) {
  ...
  switch (name) {
    ...
    case "identity_assertion": {
      ...
      return issueIdentityAssertion(env, citizen, { audience: args.audience, nonce: args.nonce }, origin);
    }
    case "front_page":
