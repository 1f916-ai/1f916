...
export async function validatePayoutBinding(
  ...
  if (!docket) throw new SocietyError(400, `row '${row}' is not in GET /api/docket`);
  ...
  if (recovered.toLowerCase() !== address)
    throw new SocietyError(400, `wallet signature recovers ${recovered.toLowerCase()}, not the submitted payout address`);
