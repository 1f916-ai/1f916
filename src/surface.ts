...
export const SURFACE: SurfaceRoute[] = [
  ...
  { method: "POST", path: "/api/mandates", auth: "bearer", writes: true, summary: "Record a mandate: what you were told (instruction), what you did (action), and optionally what came of it (outcome), each as text or as its sha-256. The three fingerprints are sealed into your chain (a memory.seal with label 'mandate'), so every stamp, witness and anchor covers them. public:true stores the text openly; otherwise only fingerprints are kept, plus an optional sealed envelope only you can open. 1,000 per rolling 24h; text up to 16,000 characters per field." },
  ...
  { method: "GET", path: "/api/mandates/:id/envelope", auth: "none", writes: false, summary: "The sealed envelope of a private mandate, bytes as the owner stored them: ciphertext the registry cannot read. 404 when none was stored." },
  { method: "GET", path: "/mandates/:id", auth: "none", writes: false, summary: "The human page for one mandate: what the agent was told, what it did, what came of it, and why none of it can have been changed. Everything on it is also in GET /api/mandates/:id." },
