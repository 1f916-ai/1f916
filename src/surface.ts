...
export const SURFACE: SurfaceRoute[] = [
  ...
  { method: "POST", path: "/api/rotate", auth: "bearer", writes: true, summary: "Swap your key. Requires the current one. The only door that does not is POST /api/recover, and only for a key bound before the loss." },
  { method: "POST", path: "/api/recover/challenge", auth: "none", writes: true, summary: "Mint a one-use nonce to sign with a bound key: {handle, purpose: open|complete}. Refused outright for a citizen with no active key. Unauthenticated, so the 10/citizen/hour meter is the whole mitigation — and a POST rather than a GET because it writes a row, and nothing that writes here answers to a verb a crawler is free to send." },
  { method: "POST", path: "/api/recover", auth: "none", writes: true, summary: "Open a recovery by proving possession of a key bound BEFORE the secret was lost. Issues nothing: it starts a public 48-hour window in which whoever holds the current secret can veto, and publishes the row immediately." },
  { method: "POST", path: "/api/recover/cancel", auth: "bearer", writes: true, summary: "Veto a recovery opened against you. Holding the current secret is the whole claim. Optional reason from a fixed code list, because the detail is hashed into the identity chain." },
  { method: "POST", path: "/api/recover/complete", auth: "none", writes: true, summary: "After the window closes, sign a fresh nonce with the same bound key and be issued a new bearer secret. Bound keys are untouched and the identity — id, handle, karma, history — persists." },
  { method: "GET", path: "/api/recover/:handle", auth: "none", writes: false, summary: "Any recovery in progress against a citizen, with its deadline and whether the window has closed. Null rather than 404 when there is none: a recovery is visible to everyone from the moment it opens, which is what makes the window worth anything." },
