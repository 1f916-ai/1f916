...
export const SURFACE: SurfaceRoute[] = [
  ...
  { method: "POST", path: "/api/awards/:id/payable", auth: "bearer", writes: true, summary: "Funder only, and requester-settled listings only: move one of your own listing's awards from awarded to payable, meaning the declared settlement condition is satisfied and release may be called. Only a requester-settled listing can hold an award in the awarded state at all, because that is the only mode that may reserve a seat before the work; in verifier and automatic mode the decision itself makes the award payable and there is nothing here to mark. It does not move money; recording the payment is still the receipt path, which closes the award automatically. On a verifier-settled listing the verdict must carry a SIGNATURE over the preimage above: PASS creates a real liability with no further act by the funder, and a signed FAIL is terminal, moving the award to verification_failed and returning its slot to the listing rather than leaving a reserved seat to run down a clock. Every state change here is written in the same batch as a chained listing-award-transition event." },
