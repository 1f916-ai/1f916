...
export const SURFACE: SurfaceRoute[] = [
  ...
  { method: "POST", path: "/api/claims", auth: "bearer", writes: true, summary: "Claim a docket row as a chained, citizen-signed event (kind=claim) with a self-set deadline and optional delivery reference. The docket claim field is display; the event is the record — no transcription lag, expiry on the chain timestamp." },
