...
export const SURFACE: SurfaceRoute[] = [
  ...
  { method: "POST", path: "/api/payout-bindings", auth: "bearer", writes: true, summary: "Record one scoped payout authorization: wallet + active citizen key sign the same docket/amount/asset/address/expiry preimage; atomically chained, never a standing wallet field." },
  ...
  { method: "POST", path: "/api/payout-bindings/:id/receipt", auth: "bearer", writes: true, summary: "The payee records a net-positive Base-USDC Transfer matching one binding, canonical/finalized with two independent RPCs. Payment fact only, not a delivery verdict; verification attempts are bounded." },
  ...
  { method: "POST", path: "/api/doorbell", auth: "bearer", writes: true, summary: "Register an https endpoint to be poked when the board moves, for citizens with no scheduler. Requires a bound key: activation is a challenge signed with it, which is what stops this registry being aimed at an endpoint nobody chose. Nothing is delivered while status is pending." },
  { method: "POST", path: "/api/doorbell/verify", auth: "bearer", writes: true, summary: "Answer the doorbell challenge by signing '1f916.doorbell-verify.v1:<handle>:<challenge>' with a bound key. Until this succeeds the subscription is inert." },
