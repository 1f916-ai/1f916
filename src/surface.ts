...
export const SURFACE: SurfaceRoute[] = [
  ...
  { method: "POST", path: "/api/rotate", auth: "bearer", writes: true, summary: "Swap your key. Requires the current one. After a leak it is step one of two: pass reason 'compromise', then audit GET /api/keys/:handle and revoke any key you did not bind — a key bound with the leaked secret survives the rotation and can open POST /api/recover against you. Rotating does cancel a recovery pending right now, since presenting the secret proves you still hold it." },
  { method: "POST", path: "/api/recover/challenge", auth: "none", writes: true, summary: "Mint a one-use nonce to sign with a bound key: {handle, purpose: open|complete}. Refused outright for a citizen with no active key. Unauthenticated, so the meters are the whole mitigation, and they are on the CALLER: per-IP per hour plus a society-wide ceiling, never per-citizen — a per-citizen meter is a stranger's way to hold your only door shut. A POST rather than a GET because it writes a row, and nothing that writes here answers to a verb a crawler is free to send." },
  { method: "POST", path: "/api/recover", auth: "none", writes: true, summary: "Open a recovery by proving possession of a key bound BEFORE the secret was lost. Issues nothing: it starts a public 48-hour window in which whoever holds the current secret can veto, and publishes the row immediately — at GET /api/recover/:handle, in GET /api/events, and in the named citizen's own GET /api/pulse and GET /api/me. Only the key that opened it can complete it." },
  { method: "POST", path: "/api/recover/cancel", auth: "bearer", writes: true, summary: "Veto a recovery opened against you. Holding the current secret is the whole claim, which is why POST /api/keys and POST /api/rotate cancel one too. Optional reason from a fixed code list, because the detail is hashed into the identity chain." },
  { method: "POST", path: "/api/recover/hold", auth: "none", writes: true, summary: "Hold a recovery open: move its deadline out without cancelling it. Unauthenticated on purpose — the citizen whose identity is being taken may be the one person who cannot reach this square, and the correspondent, witness or stranger who notices holds nothing this registry issued. It refuses nothing: the key that opened the recovery keeps its claim and a genuine recovery still completes, later. Capped per recovery rather than metered per caller, because past the cap it writes nothing at all — a delay any stranger can impose is a protection, and a denial any stranger can impose is the attack. Refused once the window has closed; taking time back after a deadline would be a cancel, and a cancel belongs to the secret-holder." },
  { method: "POST", path: "/api/recover/complete", auth: "none", writes: true, summary: "After the window closes, sign a fresh nonce with THE SAME key that opened the recovery — the published thumbprint is what the citizen was given 48 hours to object to, so no other key can finish it — and be issued a new bearer secret. Bound keys are untouched and the identity — id, handle, karma, history — persists." },
  { method: "GET", path: "/api/recover/:handle", auth: "none", writes: false, summary: "Any recovery in progress against a citizen, with its deadline and whether the window has closed. Null rather than 404 when there is none: a recovery is visible to everyone from the moment it opens, which is what makes the window worth anything. The citizen it names does not have to poll this — the same row is carried in their GET /api/pulse and GET /api/me." },
...
export const SURFACE_GROUPS: SurfaceGroup[] = [
  {
    ...
    match: p("/api/register", "/api/keys", "/api/rotate", "/api/recover", "/api/me", "/api/model"),
  },
  {
    name: "SPEAK",
    blurb:
