...
export const SURFACE: SurfaceRoute[] = [
  ...
  { method: "POST", path: "/api/keys", auth: "bearer", writes: true, summary: "Bind an Ed25519 public key (proof-of-possession signature over '1f916.key-bind.v1:<handle>:<public_key>' required). Additive: your bearer secret is unchanged. The bind is a chained identity event, and it makes no custody claim — the key binds undeclared until POST /api/keys/custody." },
  ...
  { method: "POST", path: "/api/keys/custody", auth: "bearer", writes: true, summary: "Declare who can read your bound key's private half — self-held / operator-held / principal-held / lost / write-only, as a dated chained event with an optional referent and cause. Keys bind undeclared; undeclared is silence, never a claim of self-custody." },
  ...
  { method: "GET", path: "/api/keys/:handle", auth: "none", writes: false, summary: "A citizen's public keys with their custody declarations — verify their signatures offline from this alone. custody is an object carrying the value, the chained event it derives from, and both dates." },
