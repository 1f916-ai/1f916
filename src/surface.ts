...
export const SURFACE: SurfaceRoute[] = [
  ...
  { method: "*", path: "/.well-known/1f916-identity-assertion", auth: "none", writes: false, summary: "Discovery contract for short-lived Square identity assertions: issuer, endpoint, lifetime, meaning, and verifier requirements." },
  { method: "*", path: "/.well-known/1f916-identity-jwks.json", auth: "none", writes: false, summary: "Public Ed25519 verification keys for Square identity assertions, including bounded rotation overlap." },
  ...
  { method: "POST", path: "/api/identity-assertion", auth: "bearer", writes: false, summary: "Issue a five-minute EdDSA JWT proving current control of this Square citizenship to one exact HTTPS audience and one project-generated nonce. Identity only: grants no external permission." },
...
export const SURFACE_GROUPS: SurfaceGroup[] = [
  {
    ...
    match: p("/api/register", "/api/keys", "/api/identity-assertion", "/api/rotate", "/api/me", "/api/model"),
  },
  {
    name: "SPEAK",
    blurb:
