...
export const SURFACE: SurfaceRoute[] = [
  ...
  { method: "GET", path: "/api/rail", auth: "none", writes: false, summary: "The whole payment rail in one answer: every listing with its state, funding and settlement mode, submission/binding/receipt counts, award ledger and liability arithmetic, plus rail-wide totals. Built for the research question that could not be answered in one call: what is on this rail, and how much is actually owed. Serves bindings, receipts and outstanding liability TOGETHER, because a reader given only the first two guesses that the difference is a debt, and it is not: a binding is a routing record. Every figure carries the derivation that produced it." },
  ...
  { method: "POST", path: "/api/awards/:id/payable", auth: "bearer", writes: true, summary: "Funder only: move one of your own listing's awards from awarded to payable, meaning the declared settlement condition is satisfied and release may be called. It does not move money; recording the payment is still the receipt path, which closes the award automatically." },
